...
#define INSTALL_SOURCE_DRIVER(klass, type)                                     \
  ...
  extern "C" EXPORTIT void register_pugg_plugin(pugg::Kernel *kernel) {        \
    if (!kernel->add_driver(new klass##Driver())) {                            \
      throw std::runtime_error("Failed to add source driver");                 \
    }                                                                          \
  }
...
#define INSTALL_FILTER_DRIVER(klass, type_in, type_out)                        \
  ...
  extern "C" EXPORTIT void register_pugg_plugin(pugg::Kernel *kernel) {        \
    if (!kernel->add_driver(new klass##Driver())) {                            \
      throw std::runtime_error("Failed to add filter driver");                 \
    }                                                                          \
  }
...
#define INSTALL_SINK_DRIVER(klass, type)                                       \
  ...
  extern "C" EXPORTIT void register_pugg_plugin(pugg::Kernel *kernel) {        \
    if (!kernel->add_driver(new klass##Driver())) {                            \
      throw std::runtime_error("Failed to add sink driver");                   \
    }                                                                          \
  }
